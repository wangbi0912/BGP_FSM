...
t_source = []
t_target = []
# t_source = sortedNode               #98个source
# t_target = sortedNode[::-1]         #target为source反转

for i in range(49):     #49对点对点
...
f1 = open("udp_traffic0630.txt","w")
for i in range(len(t_source)):
    f1.write("**.HA" +str(t_source[i])+ ".numApps = 2\n")
    ...
    f1.write("\n")
    f1.write("**.HA" +str(t_target[i])+ ".numApps = 2\n")
    f1.write("**.HA" +str(t_target[i])+ ".app[*].typename = \"UdpSink\"\n")
    f1.write("**.HA" +str(t_target[i])+ ".app[1].localPort = 5678\n")
    f1.write("\n")

f2 = open("vbetween0630.txt","w")
